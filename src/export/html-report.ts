...
function generateDetailReport(data: ReportData, outputDir: string): string {
  ...
  dailyData.forEach((d, i) => {
    ...
    bars += `<rect x="${x}" y="${y}" width="${barW}" height="${h}" rx="2" fill="#B45309" opacity="0.75"><title>${d.date}: ~${d.co2.toFixed(1)}g CO\u2082</title></rect>`;
...
<div class="report">
  ...
  <div class="header">
    ...
    <div class="header-sub">Estimated CO\u2082 footprint from AI-assisted development</div>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:20px">
      ...
      <table class="data-table">
        <thead><tr>
          ...
          <th style="text-align:right">~CO\u2082</th><th style="text-align:right">Share</th><th style="text-align:right">Energy Rate</th>
        </tr></thead>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;overflow:hidden">
      <table class="data-table" id="sessionTable">
        <thead><tr>
          ...
          <th data-col="5" data-type="num" style="text-align:right">~CO\u2082<span class="sort-arrow">&#9650;</span></th>
  ...
  <div class="section">
    ...
    <details class="method-details">
      ...
      <div class="inner">
        <div class="formula-box">CO\u2082 (gCO\u2082e) = (tokens x Wh/token x PUE) / 1000 x carbon_intensity

Where:
  tokens           = input + output + cache_read + cache_write
  Wh/token         = model-specific (see table below)
  PUE              = ${PUE} (datacenter overhead multiplier)
  carbon_intensity = ${intensity} gCO\u2082/kWh (${region.toUpperCase()})</div>
    ...
    <details class="method-details">
      ...
      <div class="inner">
        <table class="method-table">
          <thead><tr><th>Region</th><th style="text-align:right">gCO\u2082/kWh</th></tr></thead>
  ...
  <div class="footer">
    ...
    <div>Sources: IEA 2023, Luccioni et al. 2023, Mamun et al. 2026 (arXiv:2604.02776) | Token-based CO\u2082 is an approximation (R\u00B2 \u2248 0.44)</div>
  </div>
      ...
      <table style="width:100%;border-collapse:collapse;font-size:13px">
        <thead>
          <tr style="background:#1a1a1a;border-bottom:1px solid #333">
            ...
            <th style="padding:10px 12px;text-align:right;font-size:11px;color:var(--dim);font-weight:600;text-transform:uppercase">~CO\u2082</th>
...
CO\u2082 (gCO\u2082e) = (tokens x Wh/token x PUE) / 1000 x carbon_intensity

Where:
  tokens          = input + output + cache_read + cache_write
  Wh/token        = model-specific (see table)
  PUE             = ${PUE} (datacenter overhead)
  carbon_intensity = ${intensity} gCO\u2082/kWh (${region.toUpperCase()})
        </pre>
        <p style="margin-top:8px">Token count explains ~44% of energy variance (R\u00B2\u22480.44). Inference time is a stronger predictor but unavailable via API.</p>
      </div>
    </details>
      ...
      <div style="padding:0 20px 20px">
        <table style="width:100%;border-collapse:collapse;font-size:13px;margin-top:8px">
          <thead><tr style="border-bottom:1px solid #333">
            ...
            <th style="text-align:right;padding:6px 12px;color:var(--dim);font-size:11px;text-transform:uppercase">gCO\u2082/kWh</th>
          </tr></thead>
...
<div class="receipt-wrap">
  <div class="receipt">
    ...
    <!-- Region -->
    <div class="section-hdr">REGION: ${userRegion.toUpperCase()} (${userIntensity} gCO\u2082/kWh)</div>
